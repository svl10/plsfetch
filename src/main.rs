fn main() {
    ...
    let name: String = System::name().unwrap();
    let host: String = System::host_name().unwrap();
    let gpu: String = get_gpus()[0].clone();
    ...
    println!(
        "Display: {} x {} @ {}hz",
        display_infos[0].width, display_infos[0].height, display_infos[0].frequency
    );
